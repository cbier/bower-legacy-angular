    ...
    function createInternalInjector(cache, factory) {
      ...
      function invoke(fn, self, locals) {
        ...
        for (i = 0, length = $inject.length; i < length; i++) {
          key = $inject[i];
          args.push(
            locals && locals.hasOwnProperty(key) ? locals[key] : getService(key, path)
          );
        }
        if (!fn.$inject) {
      ...
      function ($injector, $interpolate, $exceptionHandler, $http, $templateCache, $parse,
        ...
        function compile($compileNode, transcludeFn, maxPriority) {
          ...
          return function (scope, cloneConnectFn) {
            ...
            var $linkNode = cloneConnectFn ? JQLitePrototype.clone.call($compileNode) // IMPORTANT!!!
            : $compileNode;
        ...
        function compileNodes(nodeList, transcludeFn, $rootElement, maxPriority) {
          ...
          for (var i = 0; i < nodeList.length; i++) {
            ...
            childLinkFn = (nodeLinkFn && nodeLinkFn.terminal) ? null : compileNodes(nodeList[i].childNodes,
  ...
  function lex(text, csp) {
    ...
    function throwError(error, start, end) {
      ...
      throw Error("Lexer Error: " + error + " at column" +
        (isDefined(start) ? "s " + start + "-" + index + " [" + text.substring(start, end) + "]" : " " + end) +
        " in expression [" + text + "].");
    }
        ...
        if (selector === true || scope.$index % 2 === selector) {
          if (oldVal && (newVal !== oldVal)) {
            if (isObject(oldVal) && !isArray(oldVal))
              oldVal = map(oldVal, function (v, k) {
                if (v) return k
              });
            element.removeClass(isArray(oldVal) ? oldVal.join(' ') : oldVal);
          }
          if (isObject(newVal) && !isArray(newVal))
            newVal = map(newVal, function (v, k) {
              if (v) return k
            });
